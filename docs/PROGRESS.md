...
### 2026-06-27 — Faza 8: Okrągłe przyciski (HeaderBtn/MuteButton) — bug ośmiokąta + gradient radialny; DeckCard.tsx skonwertowany
Po konwersji `HeaderBtn` (⚙️/← w `PageHeader`) i `MuteButton` (też używany jako mute+pauza w HUD gry — to ten sam komponent) na obrazki, okrągłe przyciski renderowały się jako **ośmiokąt**, nie koło — widoczne tylko na mobile, nie na web. Trzy kolejne, błędne hipotezy (usunięcie zagnieżdżonego `overflow:hidden`, `borderRadius` wprost na `Image`, spłaszczenie zagnieżdżonych clipów) dawały bajt-w-bajt identyczny render — dowód, że problem nie był tam, gdzie szukałem. Właściwa przyczyna, znaleziona przez bezpośredni test A/B: **`elevation` (cień Material na Androidzie) na widoku idealnie okrągłym renderuje swój outline jako przybliżony wielokąt**, niezależnie od clippingu dzieci. Fix: `elevation: 0` na `btnShadow`/`shadow` w obu komponentach (iOS bez zmian, ma swój `shadowColor/Offset/Opacity/Radius`).

Po naprawie kształtu — kolejne nieporozumienie: użytkownikowi chodziło o **gradient radialny** (od środka do brzegu, efekt kuli/wypukłości), nie o pionowy gradient przycięty do koła (który wizualnie nie daje żadnego efektu 3D na idealnym kole). Dodano `generateRadialGradient`/`writePng2D` do `scripts/generate-gradients.js` (odległość od centrum jako `t`, nie pozycja Y). Ważna nauka: **pierścień przy brzegu koła ma dużo większą powierzchnię niż środek**, więc to KOLOR BRZEGU dominuje odbierany ogólny ton przycisku — convex (stan spoczynku) potrzebuje jasnego brzegu, nie jasnego środka. Rozmiar źródła: radialne gradienty nie kompresują się tak dobrze jak pionowe (brak powtarzających się wierszy) — 256×256 dawało ~40KB/obrazek; finalnie 64×64 (~5.5KB/obrazek), bo to tylko ok. 52dp przycisk z jednolitym (nie dysproporcjonalnym) skalowaniem w obu osiach.

**`DeckCard.tsx`** (karty na ekranie wyboru talii) — bevel/tint zależą od `deck.color`, więc nie da się upiec JEDNEGO obrazka jak wcześniej. Ale `decks.ts` ma tylko 6 talii = 6 unikalnych kolorów (stały, znany zbiór) → wypieczony jeden bevel+tint per kolor (`deckcard_bevel_<hex>`, `deckcard_tint_<hex>`), plus jeden wspólny depth overlay i PRO badge (kolor-niezależne). `DeckCard.tsx` ma fallback na żywy `LinearGradient`, jeśli kolor talii nie jest w wypieczonym zbiorze (np. nowa talia dodana przed ponownym uruchomieniem skryptu) — żadnego ryzyka złamania przyszłych talii. `decks.tsx`'s "Odblokuj Premium" banner też skonwertowany (statyczny, jedna instancja). Źródło 1024×112 (dopasowane do realnej wysokości karty ~112px).
