...
export class GameEngine {
  ...
  getStats(): GameStats {
    ...
    return { correctCount, skippedCount, totalWords, accuracy, averageTimePerWord };
  }

  startGame(deck: Deck): void {
    if (this.state.status === 'playing') this.stopTimer();
    ...
    this.emit({ type: 'WORD_CHANGED', word: words[0], index: 0 });
  }
  ...
  private recordAnswer(wasCorrect: boolean): void {
    ...
    const result: RoundResult = { word: currentWord, wasCorrect, timestamp: Date.now() };
  ...
  private startTimer(): void {
    this.timerInterval = setInterval(() => {
      ...
      if (this.state.timeRemaining <= 0) this.finishGame();
    }, 1000);
  }
